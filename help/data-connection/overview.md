...
| [Adobe [!DNL Journey Optimizer]](https://experienceleague.adobe.com/docs/journey-optimizer/using/get-started/get-started.html) | Orchestrazione di campagne tra canali diversi | **Percorsi basati sul comportamento**: un cliente che ha acquistato un telefono cellulare due anni fa può essere preso di mira per l’acquisto di un nuovo telefono cellulare, in quanto probabilmente ora starebbe cercando un nuovo telefono cellulare. Inoltre, campagne e promozioni personalizzate possono essere utilizzate per il targeting di tali clienti. [!DNL Journey Optimizer] può utilizzare le funzionalità e-mail e SMS per raggiungere questi clienti. Per identificare le tendenze è possibile utilizzare i dati cronologici degli ordini insieme ai dati comportamentali. Un cliente che ha acquistato un articolo con una particolare configurazione in passato e che ora intende acquistare nuovamente lo stesso prodotto può migliorare il proprio percorso di acquisto fornendo una facile visibilità/accesso alle stesse configurazioni di prodotto.<br> **Personalizzazione**: con accesso alle informazioni del profilo cliente, [!DNL Journey Optimizer] può sbloccare percorsi altamente personalizzati che consentono ai commercianti di raggiungere i clienti su più canali diversi.<br> **Nuovo profilo creato**: le e-mail di benvenuto e le attività promozionali possono incoraggiare e influenzare i nuovi clienti nei loro percorsi.<br> **Profilo eliminato**: i commercianti possono scegliere di interrompere l’invio di e-mail promozionali ai clienti che hanno chiuso il loro account. In alternativa, i commercianti possono anche creare campagne per recuperare i clienti persi. |

## Estrarre dati di Experience Platform in Commerce

Invio dei dati di Commerce all’Experience Platform tramite [!DNL Data Connection] l’estensione è un lato delle funzionalità di condivisione dati di Commerce. L’altro lato, che è un’estensione facoltativa, è denominato [Audience Activation](https://experienceleague.adobe.com/docs/commerce-admin/customers/audience-activation.html). Questa estensione consente di creare tipi di pubblico in Real-Time CDP e distribuirli nel tuo archivio Commerce per informare le regole di prezzo del carrello e i blocchi dinamici.
...
* **Dati** : i dati che vengono condivisi con l’Experience Platform sono dati raccolti dagli eventi del browser nella vetrina ed eventi di back office sul server. Gli eventi di vetrina vengono acquisiti dalle interazioni degli acquirenti sul sito e includono eventi come [`addToCart`](events.md#addtocart), [`pageView`](events.md#pageview), [`createAccount`](events.md#createaccount), [`editAccount`](events.md#editaccount), [`startCheckout`](events.md#startcheckout), [`completeCheckout`](events.md#completecheckout), [`signIn`](events.md#signin), [`signOut`](events.md#signout)e così via. Consulta [eventi vetrina](events.md#storefront-events) per l’elenco completo degli eventi storefront. Gli eventi lato server o back office includono [stato ordine](events.md#back-office-events) informazioni quali [`orderPlaced`](events.md#orderplaced), [`orderReturned`](events.md#orderitemreturncompleted), [`orderShipped`](events.md#ordershipmentcompleted), [`orderCancelled`](events.md#ordercancelled)e così via. Consulta [eventi di back office](events.md#back-office-events) per l&#39;elenco completo degli eventi di back office.
